...
static void insert_main_arq(unsigned rng, unsigned low, unsigned fl, unsigned fh, int s, int nsyms, int add_code){
     ...
     if(add_code == 0){
          ...
          fprintf(arq_main, "1;%"PRIu32";%"PRIu32";%"PRIu32";%"PRIu32";%"PRIu32";%"PRIu32";", rng, low, fl, fh, s, nsyms);
     }else if(add_code == 2){
          fprintf(arq_main, "%"PRIu32";%"PRIu32";", rng, low);
     }else if(add_code == 3){
          fprintf(arq_main, "%"PRIu32";%"PRIu32";\n", rng, low);
     }
     // else{      // print the buffer output for the input
     //      fprintf(arq_main, "%"PRIu32";", rng);
     // }
     fclose(arq_main);
}
static void od_ec_enc_normalize(od_ec_enc *enc, od_ec_window low,
       ...
       int c;
       int s;
       uint16_t temp_output_bitstream;
       // unsigned binary_number, temp_binary_1 = (uint16_t)0, temp_binary_2 = (uint16_t)0;
       insert_main_arq(rng, low, 0, 0, 0, 0, 2);
  ...
  if (s >= 0) {
     ...
     enc->rng = rng << d;
     enc->cnt = s;
     insert_main_arq(enc->rng, enc->low, 0, 0, 0, 0, 3);
}
